...
####cx cy update functions in case it is in vicinity
        ...
        print v
        plt.plot(cx,cy,'bo')
        plotx = [v[0][1],v[1][1],v[2][1],v[3][1]];
        ploty = [v[0][0],v[1][0],v[2][0],v[3][0]];
        plt.plot(plotx,ploty,'r+')
##        plt.xticks(np.arange(min(plotx), max(plotx)+1, 1.0))
##        plt.yticks(np.arange(min(ploty), max(ploty)+1, 1.0))
        plt.pause(0.5)
        caught(cx,cy,v);
    else:
